...
/                          — Next.js frontend
  app/                     — pages: login, dashboard, test/[id], results/[id],
                             coordinator (+live/[id], +tests/[id]/questions),
                             admin, teacher/calendar
  components/              — ui/ (design-system primitives), dashboard/,
                             coordinator/, questions/, teacher/
...
| Teacher | `vimala@kju.edu` (Aptitude, MCA A) |
| Teacher | `vinothina@kju.edu` (Programming Fundamentals, MCA B) |
...
- Any nav bar / cross-role navigation UI (roles currently reach their pages via direct URLs post-login)
